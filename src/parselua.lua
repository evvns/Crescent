    ...
    local st, err = pcall(function()
        ...
        while true do
            ...
            elseif Digits[c] or (peek() == '.' and Digits[peek(1)]) then
                --number const
                local start = p
                if c == '0' and string.lower(peek(1)) == 'x' then
                    get();get()
    ...
    if type(src) == 'string' then
        ...
        for line in src:gmatch("[^\n]*\n?") do
            ...
            if lineNum == tok:Peek().Line then
                ...
                err = err.."   ^^^^\"n" .. tostring(lastline or "")
                break
            end
            lastline = line
        end
...
local function CreateScope(parent)
    ...
    function scope:ObfuscateVariables()
        for _, var in pairs(scope.LocalList) do
            local id = ""
            repeat
                id = string.gsub(string.rep(".", math.random(6, math.random(25, math.random(30, math.random(35, 40))))), ".", function()
